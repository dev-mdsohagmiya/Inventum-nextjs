...
import FreeCalculator from "./our-packages/FreeCalculator";
import PackageCards from "./our-packages/PackageCards";
import TabNavigation from "./our-packages/TabNavigation";
      ...
      <div className={`justify-center mb-8 flex w-full`}>
          <div className="inline-flex flex-wrap gap-6 md:flex-nowrap justify-center text-center md:gap-0 bg-[#111827] rounded-full">
            {options.map((option, index) => (
              <button onClick={()=>{setPayment(index)}} key={index} 
              ...
              >
                <span className="font-medium text-white">
                  {option.term === "Upfront" ? "Upfront" : option.term + " payments"}
                </span>
                {option.discount && (
