...
export default function TechGrid(props) {
    const [showAllTechs, setShowAllTechs] = useState(props.topTechs === undefined ? true : false);
    ...
    return (
        <div className={props.className ?? "bg-dl"}>
            <div className=' grid gap-10 mx-auto flex-wrap justify-center py-10 customgrid'>
            {
                ...
                Techjson && Techjson.map((subject, index) => {
                    return <TechCards bg={props.bg} id={`${index}`} flipindex={`${index}`} key={index} name={subject.Course} img={subject.img} text={subject.Description} position={subject.position}/>

                })
                :
                Techjson && filteredTechs.map((subject, index) => {
                    return <TechCards bg={props.bg} id={`${index}`} flipindex={`${index}`} key={index} name={subject.Course} img={subject.img} text={subject.Description} position={subject.position}/>
            ...
            :  showAllTechs === false ? (
                ...
                className={`w-72 h-40 hover:bg-pg hover:bg-clip-text hover:text-transparent arrow-hover-filter text-ll`}>
                    ...
                    <h5 className="mb-2 text-lg font-bold tracking-tight">Show All Modules</h5>
                </button>
            ) : showAllTechs === true 
            ? 
            <button 
                ...
                className={`w-72 h-40 hover:bg-pg hover:bg-clip-text hover:text-transparent arrow-hover-filter text-ll`}>
                    ...
                    <h5 className="mb-2 text-lg font-bold tracking-tight ">Show Less Modules</h5>
                  </button>
            : 
