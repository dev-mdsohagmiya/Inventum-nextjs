...
export default function ISFSubjects(props) {
    const [showAllSubjects, setShowAllSubjects] = useState(props.topSubjects === undefined ? true : false);
  ...
  const filteredSubjects = ISFsubjects.filter(subject =>
    ...
    return (
        <div className={`items-center flex flex-col w-full bg-dl ${InView ? 'animate shown' : 'animate'}`}>
            {props.notitle ? <></>:<h1 className="text-7xl text-center flex self-center py-10 font-bold bg-clip-text text-transparent bg-pg w-max">Subjects</h1>}
            ...
            ? ISFsubjects.map(subject => {
                ...
                return (
                    ...
                    <SubjectCards
                        ...
                        img={subject.img}
                        text={subject.text}
                        category={props.notitle ? 'learn/middleschool' : 'middleschool'}
                    />
                    </div>
                );
                })
            ...
            :  showAllSubjects === false ? (
                ...
                className={`w-72 h-40 hover:bg-pg hover:bg-clip-text hover:text-transparent arrow-hover-filter text-ll`}>
                    ...
                    <h5 className="mb-2 text-lg font-bold tracking-tight">Show All Subjects</h5>
                </button>
            ...
            <button 
                ...
                className={`w-72 h-40 hover:bg-pg hover:bg-clip-text hover:text-transparent arrow-hover-filter text-ll`}>
                    <img draggable={false} alt='arrow' className="-rotate-180 md:-rotate-90 brightness-200 mx-auto w-20" width={'60rem'} src={arrowTop}></img>       
                    <h5 className="mb-2 text-lg font-bold tracking-tight ">Show Less Subjects</h5>
                  </button>
            : 
