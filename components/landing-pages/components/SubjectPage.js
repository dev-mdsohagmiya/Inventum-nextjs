...
export default function SubjectPage() {
    ...
    <div className='flex flex-col min-h-screen bg-dl'>
        <div className='mt-20 mb-10 p-10 bg-dd rounded-lg text-ll flex gap-3'>
            <Link to={`/`+previousPage}><span className='text-ld hover:text-dl'>Home</span></Link>
            <p>{">"}</p>
            <Link to={`/${previousPage}/${b.slice(-2, -1).toString()}/${c}`}><span className='text-ld hover:text-dl'>{levelText} {c.replaceAll('-', ' ')}</span></Link>
        </div>
