  ...
  return (
    <div className={`${props.dark ? "bg-dd" : "bg-dl"}`}>
      ...
      <p className="max-w-prose mx-auto text-center text-ll text-xl pb-10 px-5">
        We are the only school to offer over 30 cutting-edge technology modules
        ensuring our students are equipped with the knowledge and skills to
        thrive in the rapidly evolving technological landscape.
      </p>
      <TechGrid
        bg={props.dark ? "bg-dl" : "bg-dd"}
        className={`${props.dark ? "bg-dd" : "bg-dl"}`}
        topTechs={[
